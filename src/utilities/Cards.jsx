...
const CardContainer = ({ children, mobile, desc, form, className, classFlex }) => {
	const size = desc ? (!mobile ? '!w-[55%]' : '!w-full') : (!mobile ? '!w-[45%]' : '!w-full')
	const classes = className + ' bg-[#FCFEFE] p-10 rounded-md shadow-md gap-10 ' + size
	return (
		<div className={classes}>
			<FlexCol className={'!items-start !justify-center !space-y-2 gap-10 '+classFlex}>
				{children}
			</FlexCol>
		</div>
	)	
